...
class FFModel(BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        return self.sess.run([self.next_obs_pred], feed_dict={self.obs_pl: observations, self.acs_pl: actions,  
                                                                ...
                                                                self.delta_mean_pl: data_statistics['delta_mean'], self.delta_std_pl: data_statistics['delta_std']})[0] # TODO(Q1) Run model prediction on the given batch of data

    def update(self, observations, actions, next_observations, data_statistics):
        # train the model
        _, loss = self.sess.run([self.train_op, self.loss], feed_dict={self.obs_pl: observations, self.acs_pl: actions, self.delta_labels: next_observations-observations, 
                                                                        self.obs_mean_pl: data_statistics['obs_mean'], self.obs_std_pl: data_statistics['obs_std'],
                                                                        self.acs_mean_pl: data_statistics['acs_mean'], self.acs_std_pl: data_statistics['acs_std'],
                                                                        self.delta_mean_pl: data_statistics['delta_mean'], self.delta_std_pl: data_statistics['delta_std']}) # TODO(Q1) Run the defined train_op here, and also return the loss being optimized (on this batch of data)
        return loss